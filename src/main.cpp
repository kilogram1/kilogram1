...
// Copyright (c) 2013 GramCoin developers
// Copyright (c) 2014 GramCoin developers
...
const string strMessageMagic = "GramCoin Signed Message:\n";
...
/** Amount of GramCoins spent by the transaction.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GramCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GramCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running GramCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GramCoinMiner terminated\n");
        throw;
    }
}
